	...
	list : function (mode, x, y, w, h) {
		...
		this.paint = (gr) => {
			...
			case this.mode == 'lastfm_info' && this.properties.mode.value == 1: // charts
				...
				const unit_width = (this.w - lastfm_charts_bar_x - _.scale(50)) / this.data[0].playcount;
		...
		this.update = () => {
			...
			case 'lastfm_info':
				...
				case 1:
					...
					if (_.isFile(this.filename)) {
						let data = _.get(_.jsonParseFile(this.filename), this.methods[this.properties.method.value].json, []);
		...
		this.init = () => {
			...
			case 'properties':
				...
				this.add_tech = (f) => {
					...
					const samples = _.formatNumber(panel.tf('%length_samples%'), ' ');
					this.data.push({
						name : 'DURATION',
		...
		this.artist = '';
		this.filename = '';
		this.up_btn = new _.sb(chars.up, this.x, this.y, _.scale(12), _.scale(12), () => { return this.offset > 0; }, () => { this.wheel(1); });
		this.down_btn = new _.sb(chars.down, this.x, this.y, _.scale(12), _.scale(12), () => { return this.offset < this.items - this.rows; }, () => { this.wheel(-1); });
