...
_.mixin({
	...
	drawImage (gr, img, src_x, src_y, src_w, src_h, aspect, border, alpha) {
		if (!img) {
			return [];
		}
	...
	getFiles (folder, exts, newest_first) {
		let files = [];
		if (_.isFolder(folder)) {
	...
	tf (t, metadb) {
		...
		const str = tfo.EvalWithMetadb(metadb);
		return str;
	},
	tfe (t, force) {
		let tfo = fb.TitleFormat(t);
		const str = tfo.Eval(force);
		return str;
	},
	toRGB (a) {
...
let tooltip = window.CreateTooltip('Segoe UI', _.scale(12));
