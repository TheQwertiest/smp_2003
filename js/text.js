	...
	text : function (mode, x, y, w, h) {
		...
		this.success = (f) => {
			...
			case 'allmusic':
				if (this.allmusic_url) {
					this.allmusic_url = false;
					const content = _(_.getElementsByTagName(this.xmlhttp.responseText, 'div'))
		...
		this.init = () => {
			...
			case 'allmusic':
				...
				this.tidy = (value) => {
					return panel.tf('$replace($lower($ascii(' + _.fbEscape(value) + ')), & ,, and ,)');
				}
		...
		this.down_btn = new _.sb(chars.down, this.x, this.y, _.scale(12), _.scale(12), () => { return this.offset < this.lines.length - this.rows; }, () => { this.wheel(-1); });
